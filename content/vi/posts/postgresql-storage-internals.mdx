...
- **FSM fork** (`<relfilenode>_fsm`): Free Space Map.
- **VM fork** (`<relfilenode>_vm`): Visibility Map.
...
- **`t_xmax` (4 B)** — XID đã DELETE hoặc UPDATE tuple này. Nếu = 0 nghĩa là tuple đang live, chưa bị ai xóa. Khi bạn `DELETE` một row, PostgreSQL chỉ set `t_xmax = current_xid` và để nguyên tuple ở đó — đây là bản chất của MVCC.
...
Khi tuple vượt ngưỡng này, TOAST kích hoạt: PostgreSQL nén các cột variable-length hoặc tách chúng ra 1 bảng phụ, chỉ để lại một con trỏ inline.
