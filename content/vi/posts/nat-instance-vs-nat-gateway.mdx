...
```
src: 10.0.0.20   ← IP của EC2 trong private subnet
dst: 50.60.4.10  ← IP của server bên ngoài internet
```

NAT Instance có IP `10.0.0.30`. Cả source lẫn destination đều **không phải** IP của NAT Instance. Với Source/Destination Check bật, packet này bị drop — NAT Instance không bao giờ thấy nó.

Tương tự khi NAT Instance forward packet ra ngoài, source IP ban đầu (`10.0.0.20`) không phải IP của NAT Instance → check fail → packet bị drop lần nữa.
...
| Server ngoài internet        | `50.60.4.10`                                  |

**Chiều đi (Outbound):**

| #   | Vị trí              | src                          | dst           | Hành động                                           |
| --- | ------------------- | ---------------------------- | ------------- | --------------------------------------------------- |
| 1   | **EC2**             | `10.0.0.20`                  | `50.60.4.10`  | Tạo packet                                          |
| 2   | **Route Table** (private) | `10.0.0.20`             | `50.60.4.10`  | Match `0.0.0.0/0 → eni-xxx` → forward đến NAT Instance |
| 3   | **NAT Instance**    | `10.0.0.20` → `10.0.0.30`   | `50.60.4.10`  | iptables thay src IP, lưu mapping vào conntrack table |
| 4   | **Route Table** (public) | `10.0.0.30`              | `50.60.4.10`  | Match `0.0.0.0/0 → igw-xxx` → forward đến IGW       |
| 5   | **IGW**             | `10.0.0.30` → `12.34.56.78` | `50.60.4.10`  | NAT 1:1 private → EIP                               |
| 6   | **Internet**        | `12.34.56.78`                | `50.60.4.10`  | Server nhận request                                  |

**Chiều về (Return):**

| #   | Vị trí              | src           | dst                          | Hành động                            |
| --- | ------------------- | ------------- | ---------------------------- | ------------------------------------ |
| 1   | **Server**          | `50.60.4.10`  | `12.34.56.78`                | Gửi response                         |
| 2   | **IGW**             | `50.60.4.10`  | `12.34.56.78` → `10.0.0.30` | NAT 1:1 EIP → private                |
| 3   | **Route Table** (public) | `50.60.4.10` | `10.0.0.30`              | Match `10.0.0.0/16 → local`          |
| 4   | **NAT Instance**    | `50.60.4.10`  | `10.0.0.30` → `10.0.0.20`   | Tra conntrack, thay dst IP về EC2 gốc |
| 5   | **Route Table** (private) | `50.60.4.10` | `10.0.0.20`             | Match `10.0.0.0/16 → local`          |
| 6   | **EC2**             | `50.60.4.10`  | `10.0.0.20`                  | Nhận response                        |
...
| ----- | ---- | ------------------------------------- | --------------------------------------- |
| HTTP  | 80   | `10.0.0.0/24` (Private Subnet CIDR)  | Cho phép traffic HTTP từ private subnet |
| HTTPS | 443  | `10.0.0.0/24` (Private Subnet CIDR)  | Cho phép traffic HTTPS từ private subnet |
...
**Lưu ý quan trọng về High Availability**: NAT Gateway có HA tự động **trong một AZ**. Nếu AZ đó down, NAT Gateway cũng down. Để có HA trên toàn region, bạn cần deploy **một NAT Gateway ở mỗi AZ** và cấu hình route table tương ứng cho private subnet trong AZ đó.
