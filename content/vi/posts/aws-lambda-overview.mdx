...
CloudFront là CDN của AWS: nó cache nội dung tại các điểm gần người dùng để giảm độ trễ. Nhiều khi bạn muốn **chạy một đoạn logic ngay tại
...
Để một hàm trong VPC vừa chạm được tài nguyên riêng tư, vừa ra được internet (hoặc gọi các API công khai của AWS), kiến trúc đúng là: đặt hàm trong **private subnet**, và cho subnet đó một đường đi tới NAT Gateway đặt ở public subnet. NAT Gateway lo phần ra internet. (Bài [VPC Networking](/vi/posts/aws-vpc-networking-overview) đi sâu hơn về subnet và NAT.)
