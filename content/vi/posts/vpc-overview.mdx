...
Đây chính là lý do EC2 ở private subnet không thể bị internet access trực tiếp — route table của nó không có đường đi từ IGW vào.
...
1. **NAT 1-to-1**: dịch public IP sang private IP (chiều vào), và ngược lại (chiều ra)
...
IGW là stateless — không filter, không cache, không load balance. Một VPC chỉ gắn được một IGW.
...
Nhưng một NAT Gateway chỉ có một EIP — làm sao đại diện cho hàng chục, hàng trăm service cùng lúc? Câu trả lời nằm ở **port**. NAT Gateway sử dụng kỹ thuật **PAT** (Port Address Translation — dịch địa chỉ theo port): mỗi connection từ private subnet được gán một **port riêng** trên EIP. Ví dụ, EC2 `10.0.2.50` gửi request → NAT Gateway gán port `40001`, EC2 `10.0.2.51` gửi request → gán port `40002`. Khi response trả về EIP trên port `40001`, NAT Gateway tra bảng mapping, biết port này thuộc về `10.0.2.50` và chuyển response về đúng service.
...
| Chiều           | Hai chiều (in + out)              | Một chiều (chỉ out)                      |
| Có ENI?         | Không                             | Có                                       |
| Cần Elastic IP? | Không (gắn vào resource)          | Có (bắt buộc khi tạo)                    |
