...
<ExcalidrawDiagram
  ...
  alt="AWS PrivateLink end-to-end — Consumer VPC with endpoint ENIs and private DNS, the AWS Hyperplane fabric doing SNAT/DNAT, and the Provider VPC with an NLB-fronted endpoint service"
/>
...
<ExcalidrawDiagram
  ...
  alt="Comparison — VPC Peering full mesh requiring n-squared links versus PrivateLink service hub where many consumers connect one-way to a single endpoint service"
/>
...
<ExcalidrawDiagram
  ...
  alt="AWS Hyperplane internals — a fleet of EC2-based packet forwarders per AZ, shuffle sharding mapping each endpoint to an overlapping subset of nodes, a distributed flow-state table, and SNAT/DNAT"
/>
