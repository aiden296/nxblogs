...
Amazon S3 cung cấp **7 storage classes**, mỗi class được thiết kế cho access pattern và yêu cầu chi phí khác nhau. Tất cả các class đều có cùng 11 9's durability — dữ liệu của bạn an toàn như nhau bất kể bạn chọn class nào. Sự khác biệt nằm ở **availability**, tốc độ retrieval, và **chi phí**.
...
Tương tự Standard-IA nhưng chỉ lưu trong 1 AZ thay vì >= 3. Đây là option rẻ nhất trong nhóm Standard — nhưng có rủi ro: nếu AZ đó gặp sự cố, data **tạm thời không truy cập được**, và nếu AZ bị phá hủy, data **mất vĩnh viễn**.
...
- Phù hợp cho data có thể tạo lại nếu mất (re-creatable)
...
### Flexible Retrieval hay Deep Archive?

Hai class này dễ gây nhầm lẫn vì cả hai đều rẻ và đều phục vụ archival. Khác biệt thực sự nằm ở thời gian retrieval mà yêu cầu cho phép và độ dài thời gian lưu trữ. Deep Archive có storage rẻ hơn nhưng bị giới hạn retrieval 12-48 giờ và min storage duration 180 ngày. Flexible Retrieval đắt hơn một chút về storage nhưng lấy data nhanh hơn — vài phút đến vài giờ — và chỉ cam kết tối thiểu 90 ngày.

Quy tắc nhanh: mặc định chọn **Glacier Flexible Retrieval** cho data hiếm khi truy cập, trừ khi đề nêu rõ thời hạn lưu trữ nhiều năm (thường 7-10+ năm) và chấp nhận chờ 12-48 giờ — khi đó **Glacier Deep Archive** mới là lựa chọn cost-effective nhất. Tín hiệu quyết định thường là yêu cầu retrieval trong đề: "trong vòng 12 giờ" khớp đúng mức Standard của Deep Archive nên Deep Archive là đủ, còn nếu cần lấy lại trong vài phút đến vài giờ thì Deep Archive bị loại. Trường hợp đề chỉ đưa ra một option Glacier dạng archive, hãy chọn chính option đó.
...
Điểm cần lưu ý: Intelligent-Tiering chỉ đáng dùng khi access pattern **không thể đoán hoặc đo lường trước** — bạn không biết object sẽ được truy cập thường xuyên hay hiếm khi. Vì S3 phải liên tục theo dõi từng object để chọn tier phù hợp, bạn mới phải trả thêm monitoring fee. Ngược lại, nếu pattern đã rõ ràng và dự đoán được — ví dụ object chỉ được đọc khoảng 2 lần mỗi tháng — hãy chọn thẳng Standard-IA (hoặc One Zone-IA): cùng mức giá lưu trữ rẻ tương đương mà không tốn monitoring fee.
