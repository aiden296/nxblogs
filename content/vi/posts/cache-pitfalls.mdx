...
**Định nghĩa:** Cache Penetration xảy ra khi user (hoặc hacker) liên tục query dữ liệu **không có trong Cache và cũng không có trong Database**. Ví dụ: hacker gửi hàng triệu request tìm `user_id = -1` hoặc UUID ngẫu nhiên → DB bị DDoS.
...
Bloom Filter là cấu trúc dữ liệu xác suất cực kỳ hiệu quả về bộ nhớ, trả lời theo kiểu:
