...
<ExcalidrawDiagram
  ...
  alt="Giải phẫu một load balancer: client request đi vào Load Balancer qua một Listener (cổng 443/HTTPS), Listener chứa các Listener Rules quyết định định tuyến — nếu path là /api thì tới Target Group A (containers), nếu host là img thì tới Target Group B (Lambda), mặc định tới Target Group C (EC2 instances); mỗi Target Group gắn một health check. Ghi chú: ALB ngắt kết nối và thêm header X-Forwarded-For/Proto/Port để backend vẫn thấy client gốc; bộ khung này áp dụng cho mọi loại ELB, chỉ khác phần listener thông minh tới đâu"
/>
...
<ExcalidrawDiagram
  ...
  alt="Cây quyết định chọn load balancer cho kỳ thi SAA: nếu là HTTP/HTTPS và cần định tuyến theo path/host/header, WAF hay Cognito thì chọn ALB (Layer 7); nếu không, hỏi tiếp có cần chèn dàn firewall/IDS bên thứ ba để soi traffic inline không — nếu có thì chọn GWLB (GENEVE 6081, trong suốt); nếu không, hỏi tiếp có cần TCP/UDP, IP tĩnh, hiệu năng cực cao hay PrivateLink không — nếu có thì chọn NLB (Layer 4); nếu tất cả đều không và chỉ là hệ thống legacy/EC2-Classic thì mới tới CLB, và nên migrate"
/>
