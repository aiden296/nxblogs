...
<ExcalidrawDiagram
  ...
  alt="Same table laid out two ways — row-oriented stores each full row contiguously, column-oriented stores each column contiguously; SUM(amount) only needs the amount block in the column layout"
/>
...
<ExcalidrawDiagram
  ...
  alt="Four lightweight column encodings — dictionary encoding maps values to small ids, run-length encoding stores repeated values as (value, count), delta encoding stores differences, bit-packing fits small integers into minimal bits"
/>
...
<ExcalidrawDiagram
  ...
  alt="Anatomy of a Parquet file — PAR1 magic numbers bracket the file; row groups hold one column chunk per column; each chunk holds dictionary and data pages; the footer stores schema and per-column-chunk min/max statistics and offsets"
/>
...
<ExcalidrawDiagram
  ...
  alt="Querying Parquet on S3 with Athena — Parquet files partitioned by date live in S3, Glue Data Catalog holds table schema and partitions, serverless Athena scans only needed columns and partitions billed by bytes scanned, results flow to a BI dashboard"
/>
