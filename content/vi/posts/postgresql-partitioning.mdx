...
Có một thời điểm trong vòng đời của hầu hết hệ thống production: bảng `events` (hoặc `logs`, hoặc `orders`) chạm mốc nửa tỷ dòng. Mọi thứ bắt đầu trở nên... khó chịu. Query trả về sau 30 giây thay vì 30 mili-giây. `VACUUM` chạy 6 tiếng đồng hồ và vẫn chưa xong. Bạn muốn xoá dữ liệu cũ hơn 12 tháng để giải phóng dung lượng — nhưng câu `DELETE` lại sinh ra hàng triệu **dead tuples** (xem [Deep Dive về Table Bloat](/posts/table-bloat)) khiến tình hình tệ hơn lúc chưa xoá. Indexes phình to lên 50GB. Backup mất cả đêm.
