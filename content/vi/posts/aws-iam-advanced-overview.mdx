...
<ExcalidrawDiagram
  ...
  alt="Cấu trúc cây của AWS Organization: một management account và nhiều member account, mở một member account thấy một root user và nhiều IAM user."
/>

**AWS Organizations** là dịch vụ giúp bạn gom nhiều AWS account vào một tổ chức duy nhất để quản lý tập trung: gộp hóa đơn, áp chính sách bảo mật chung, và chia nhóm account theo phòng ban/môi trường.

Hãy xem AWS Account như một **hộp chứa tài nguyên**, có billing riêng, được bảo mật kỹ càng. Khi tạo 1 AWS Account, 1 root user sẽ được tạo trong account này, đăng nhập bằng email, có quyền tuyệt đối, không giới hạn được bằng IAM Policy. Bên cạnh đó, root user có thể tạo thêm các IAM User, đăng nhập thông qua Username và Account ID.
...
- **Management account** là account "chủ" - được sử dụng để tạo organization, tạo hoặc mời các account khác, và xem hóa đơn tổng. Đây là account quyền lực nhất — nên giữ nó "sạch", chỉ dùng để quản trị, **không** chạy workload trong đó.
...
Ghép lại, toàn bộ cấu trúc xếp thành một cây: gốc là Organization, bên dưới là một management account và nhiều member account; mở một member account ra sẽ thấy một root user (sinh ra cùng account) và nhiều IAM user do bạn tạo.
...
Đây là lý do "kinh tế" để dùng Organizations. Tính năng **Consolidated Billing** cho phép:

- **Một hóa đơn duy nhất** cho toàn bộ account  — management account tổng hợp hoá đơn và trả tiền.
...
- **Chia sẻ Reserved Instances và Savings Plans**: phần Reserved Instance hoặc Savings Plans mua ở một account nhưng không dùng hết có thể được các account khác trong tổ chức tận dụng, tránh lãng phí cam kết. Mặc định luôn được **bật** trên toàn bộ member account, có thể tắt cho từng account thông qua management account.
...
- Khi bạn assume một IAM Role (qua STS `AssumeRole`), bạn **từ bỏ** quyền gốc của mình và khoác lên bộ quyền của role trong suốt phiên đó. Giống như bạn trở thành 1 con người mới hoàn toàn, quên sạch trước đây mình từng là ai.
...
**AWS IAM Identity Center** (tên cũ là **AWS SSO**) là dịch vụ cho phép người dùng đăng nhập một lần rồi truy cập mọi AWS account trong Organization lẫn các ứng dụng doanh nghiệp — tất cả từ một portal duy nhất. Đây chính là lời giải cho câu hỏi số 3 ở đầu bài: thay vì tạo 200 user × 20 account, bạn quản lý danh tính một nơi.
