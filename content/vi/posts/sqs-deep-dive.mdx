...
date: '2026-05-19'
tags: ['Devops', 'AWS', 'SAA']
description: 'Deep dive Amazon SQS — kiến trúc phân tán bên dưới, Standard vs FIFO queue, message lifecycle, visibility timeout, long polling, Dead Letter Queue, và patterns thực tế: Auto Scaling Group integration, SQS as database write buffer.'
enableComment: true
---
...
```
backlog_per_instance = ApproximateNumberOfMessages / số_instances_hiện_tại
```
...
SQS là lựa chọn đúng khi bạn cần một **task queue đơn giản, fully managed, scale vô hạn** để decouple các thành phần trong hệ thống. Nếu bạn cần **event streaming** với replay và retention dài hạn → Kafka. Nếu cần **fan-out** một event đến nhiều subscriber → SNS. Nếu cần **routing phức tạp** dựa trên event content → EventBridge. Mỗi công cụ có vị trí riêng — SQS giỏi nhất ở sự đơn giản và đáng tin cậy.
