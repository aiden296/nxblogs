...
Đây chính là bài toán mà Amazon SQS giải quyết. Một hàng đợi tin nhắn **fully managed**, không cần provisioning, không cần lo capacity planning. Bạn gửi message vào, consumer lấy ra. AWS lo toàn bộ phần availability, durability, và scaling phía dưới.
...
**Backend Storage Layer** — trái tim của SQS. Đây là một distributed key-value store. Message được lưu dưới dạng cặp key-value, trong đó key được tạo từ queue ID và message metadata, value là message body (tối đa 256KB) cùng các attributes. Điểm quan trọng: message được **shard** (phân mảnh) trên nhiều storage node dựa theo queue ID để cân bằng tải.
...
3. SQS sử dụng cơ chế quorum-based writes. Điều này đảm bảo message tồn tại ngay cả khi một AZ gặp sự cố.
...
**Bước 3 — ReceiveMessage**: Consumer poll message. SQS trả về message kèm một receipt handle.
...
**Fan-out pattern** giải quyết vấn đề này bằng cách kết hợp SNS — một dịch vụ pub/sub (publish-subscribe) cho phép gửi một message đến nhiều subscriber cùng lúc — với **SQS** để đảm bảo mỗi subscriber nhận và xử lý message một cách đáng tin cậy.
