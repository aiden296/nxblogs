...
* **DELETE:** Postgres **không xóa ngay lập tức** dữ liệu khỏi ổ cứng. Nó chỉ đánh dấu tuple đó là "đã chết" (Dead Tuple).
...
Đây là cơ chế của hệ thống MVCC. Việc giữ lại các "xác" dữ liệu cũ cho phép Postgres quản lý transaction, hỗ trợ rollback và đảm bảo tính nhất quán khi có nhiều kết nối cùng đọc/ghi một lúc (read consistency) mà không cần khóa (lock) quá chặt.
...
Cache Efficiency là khả năng database giữ dữ liệu thường xuyên truy cập trong RAM để giảm thiểu I/O từ ổ cứng. Khi dữ liệu nằm trong cache, các thao tác đọc/ghi nhanh hơn đáng kể vì truy cập bộ nhớ nhanh hơn nhiều so với truy cập ổ cứng.
