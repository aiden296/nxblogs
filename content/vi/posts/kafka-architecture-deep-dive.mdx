...
**Apache Kafka** là một distributed event streaming platform. Nói đơn giản, Kafka là một hệ thống trung gian cho phép các ứng dụng **gửi** (publish) và **nhận** (subscribe) các sự kiện theo thời gian thực, đồng thời **lưu trữ** những sự kiện đó một cách bền vững.
...
Tên của segment file chính là base offset. Khi segment đạt giới hạn kích thước (mặc định 1GB) hoặc thời gian, Kafka tạo segment mới:
...
Trong kiến trúc ban đầu, Kafka sử dụng Apache ZooKeeper để làm hai việc:
...
Từ phiên bản 3.3+, Kafka giới thiệu KRaft — một giao thức đồng thuận được tích hợp trực tiếp vào Kafka, loại bỏ hoàn toàn sự phụ thuộc vào ZooKeeper:

- **Controller election** giờ dùng Raft consensus protocol thay vì ZooKeeper's ephemeral node.
...
Không phải mọi follower đều đáng tin cậy ở mọi thời điểm. Một follower có thể bị lag vì network chậm, disk bottleneck, hoặc đang restart. Kafka theo dõi danh sách ISR.
...
Mỗi message trong partition được gán một offset. Offset chính là "vị trí đọc" của consumer trong partition log, giống như bookmark đánh dấu trang bạn đang đọc trong sách.
