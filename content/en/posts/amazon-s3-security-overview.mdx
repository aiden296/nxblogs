...
Worth remembering about how the keys work: each object is encrypted with its own unique key, not one shared key across the whole bucket. That key is itself encrypted with a root key that AWS manages and rotates periodically. This nested-key mechanism is called **envelope encryption** — use one key to encrypt the data, then use another key to encrypt that key, so rotating the root key doesn't require re-encrypting all the data.
