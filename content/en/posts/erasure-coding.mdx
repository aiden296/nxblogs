...
- **1950s**: Richard Hamming developed Hamming codes.
- **1960**: Irving Reed and Gustave Solomon published Reed-Solomon codes. Originally designed for satellite communications.
...
RAID was one of the earliest solutions:
...
**Google File System** (2003) and HDFS defaulted to 3 replicas. The advantages are clear:
...
- The **top k rows** form an identity matrix. This ensures the first k elements of the output are the original data (unchanged).
- The **bottom m rows** contain the parity coefficients, typically constructed from a Vandermonde matrix.
...
In section 4.1, addition was presented as a simple abstraction. In practice, the arithmetic doesn't use regular integers or floating-point numbers — it operates in a Galois Field.
...
Modern CPUs mitigate this with SIMD instructions. Intel's ISA-L (Intelligent Storage Acceleration Library) achieves encoding throughput of 2-10 GB/s — fast enough for most workloads, but still notably slower than raw memory copy speeds (20+ GB/s).
...
This is why erasure coding is used for object storage like S3, not for **block storage** or **databases** — where data changes constantly.
