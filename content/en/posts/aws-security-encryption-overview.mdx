...
- **Master key (KMS key)** is the root key that never leaves KMS. It does not directly encrypt your large data; its job is to encrypt and decrypt other keys.
- **Data key (Data Encryption Key)** is a key KMS generates on demand to encrypt the actual data. This is the key that works directly with your data, and it is itself protected (encrypted) by the KMS key.
...
The flow to obtain a data key via the `GenerateDataKey` API:
...
**Amazon GuardDuty** is an intelligent threat-detection service that uses machine learning, anomaly detection, and threat intelligence to find malicious activity. The big advantage: it's a one-click enable, no agents to install.
