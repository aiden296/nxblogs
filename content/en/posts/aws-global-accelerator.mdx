...
AWS measured at <Tooltip content="90th percentile — 90% of measurements fall below this value">p90</Tooltip> with real-user monitoring tools: <Tooltip content="First byte latency — the round-trip time for a packet to go from client to endpoint and back">first byte latency</Tooltip> down by up to 49%, jitter down by up to 58%, and throughput up by up to 60%. For applications that are sensitive during session setup, such as video conferencing or real-time APIs, this is a difference users feel immediately.
...
<ExcalidrawDiagram src="/blue-green-route53-vs-global-accelerator.excalidraw" alt="Blue-green deployment compared with Route 53 weighted routing and Global Accelerator" />
