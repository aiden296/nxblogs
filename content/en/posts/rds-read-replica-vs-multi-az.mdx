...
- Primary dies → AWS automatically fails over the DNS endpoint to the standby (~60-120s).
...
- **Asynchronous replication** — the primary doesn't wait for the replica to confirm, so writes are faster, but replicas always have replication lag, from a few ms to several seconds depending on load.
