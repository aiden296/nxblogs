...
<ExcalidrawDiagram
  ...
  alt="Anatomy of a load balancer: a client request enters the Load Balancer through a Listener (port 443/HTTPS); the Listener holds Listener Rules that decide routing — if the path is /api go to Target Group A (containers), if the host is img go to Target Group B (Lambda), otherwise the default goes to Target Group C (EC2 instances); each Target Group has a health check. Note: ALB terminates the connection and adds X-Forwarded-For/Proto/Port headers so the backend still sees the original client; this skeleton applies to every ELB type, only the listener intelligence differs"
/>
...
<ExcalidrawDiagram
  ...
  alt="Decision tree for choosing a load balancer for the SAA exam: if it is HTTP/HTTPS and you need routing by path/host/header, WAF, or Cognito, choose ALB (Layer 7); if not, ask whether you need to insert a fleet of third-party firewall/IDS appliances to inspect traffic inline — if yes, choose GWLB (GENEVE 6081, transparent); if not, ask whether you need TCP/UDP, a static IP, extreme performance, or PrivateLink — if yes, choose NLB (Layer 4); if none apply and it is only a legacy/EC2-Classic system, only then CLB, and you should migrate"
/>
