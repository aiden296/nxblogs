...
**Apache Kafka** is a distributed event streaming platform. In simple terms, Kafka is a middleware that allows applications to **publish** and **subscribe** to events in real time, while also **storing** those events durably.
...
The segment file name is the base offset. When a segment reaches its size limit (default 1GB) or time limit, Kafka creates a new segment:
...
In the original architecture, Kafka used Apache ZooKeeper for two things:
...
Starting with version 3.3+, Kafka introduced KRaft — a consensus protocol integrated directly into Kafka, completely eliminating the dependency on ZooKeeper:

- **Controller election** now uses the Raft consensus protocol instead of ZooKeeper's ephemeral nodes.
...
Not all followers are reliable at all times. A follower might lag due to slow network, disk bottleneck, or being in the middle of a restart. Kafka maintains a list called ISR.
...
Each message in a partition is assigned an offset. The offset is essentially the consumer's "read position" in the partition log, like a bookmark marking the page you're reading in a book.
