...
- **GCRA.** A mathematically compact variant of token bucket that only needs to store **1 number** (`theoretical arrival time`) instead of 2 (tokens + last_refill). The Redis module `redis-cell` implements exactly this.
