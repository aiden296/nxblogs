...
You've just deployed an e-commerce API on AWS. Following best practices, the EC2 instance sits in a **private subnet** — there's no direct path from the internet in. An Application Load Balancer sits in the **public subnet**, receives requests from clients, and forwards them to EC2.
...
Before the packet touches any component, it needs to know where it's going. VPC is that boundary.
...
> An interesting detail: when [EC2](/en/posts/learn-ec2-part-1) boots up, the operating system receives its IP through DHCP. But in reality, the IP was already picked by AWS **before the OS boots** — when creating the ENI, the control plane selects an IP from the subnet and saves it to the database. DHCP is merely the channel that **delivers** that IP to the OS, not the one that assigns it.
...
The packet from client `1.2.3.4` with destination `52.10.10.10` (ALB's public IP) has arrived at the AWS edge network. The first component it encounters is the Internet Gateway.
...
NAT Gateway sits in the **public subnet**, has an Elastic IP, and allows resources in private subnets to **send** requests to the internet but **blocks** the internet from sending requests in.
...
But a NAT Gateway has only one EIP — how can it represent dozens or hundreds of services simultaneously? The answer lies in **ports**. NAT Gateway uses a technique called PAT: each connection from the private subnet is assigned a **unique port** on the EIP. For example, EC2 `10.0.2.50` opens a connection to Stripe → NAT Gateway assigns port `40001`, the same EC2 opens another connection to a logging service → assigned port `40002`. When a response comes back to the EIP on port `40001`, NAT Gateway checks its mapping table, knows this port belongs to `10.0.2.50`'s Stripe connection, and forwards the response correctly.
