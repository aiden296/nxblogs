...
This is exactly why EC2 in the private subnet can't be accessed directly from the internet — its route table has no route from IGW in.
...
1. **1-to-1 NAT**: translates public IP to private IP (inbound), and vice versa (outbound)
...
IGW is stateless — no filtering, no caching, no load balancing. A VPC can only attach one IGW.
...
But a NAT Gateway has only one EIP — how can it represent dozens or hundreds of services simultaneously? The answer lies in **ports**. NAT Gateway uses a technique called **PAT** (Port Address Translation): each connection from the private subnet is assigned a **unique port** on the EIP. For example, EC2 `10.0.2.50` sends a request → NAT Gateway assigns port `40001`, EC2 `10.0.2.51` sends a request → assigned port `40002`. When a response comes back to the EIP on port `40001`, NAT Gateway checks its mapping table, knows this port belongs to `10.0.2.50`, and forwards the response to the correct service.
...
| Direction            | Bidirectional (in + out)               | One-way (out only)                           |
| Has ENI?             | No                                     | Yes                                          |
| Requires Elastic IP? | No (attached to resource)              | Yes (required at creation)                   |
