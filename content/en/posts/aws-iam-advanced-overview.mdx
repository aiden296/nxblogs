...
- **Sharing Reserved Instances and Savings Plans**: a <Tooltip content="Reserved Instance — a 1–3 year EC2 commitment in exchange for a much lower price than on-demand">Reserved Instance</Tooltip> or <Tooltip content="Savings Plans — a commitment to a fixed hourly spend over 1–3 years for a compute discount, more flexible than a Reserved Instance">Savings Plan</Tooltip> bought in one account but not fully used can be picked up by other accounts in the organization, so no commitment goes to waste. It's **on** by default for every member account, and can be turned off per account from the management account.
