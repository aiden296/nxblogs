...
<ExcalidrawDiagram
  src="/aws-organizations-account-structure.excalidraw"
  alt="AWS Organization tree: one management account and many member accounts, with one member account opened to show a single root user and many IAM users."
/>

**AWS Organizations** is the service that lets you group many AWS accounts into a single organization for central management: consolidate billing, apply shared security policies, and split accounts into groups by department/environment.

Think of an AWS account as a **resource container** with its own billing and a tightly guarded security boundary. When you create an AWS account, a root user is created inside it — you sign in with the account's email, it has absolute power, and no IAM policy can rein it in. On top of that, the root user can create IAM users, who sign in with a username and the account ID.
...
- The **management account** is the "owner" account — used to create the organization, create or invite the other accounts, and view the combined bill. It's the most powerful account — keep it "clean," use it only for administration, and do **not** run workloads in it.
...
Put together, the whole thing forms a tree: the organization sits at the root, with one management account and many member accounts beneath it; open up a member account and you find one root user (born with the account) plus the many IAM users you create.
...
This is the "economic" reason to use Organizations. The **Consolidated Billing** feature gives you:

- **A single bill** for all accounts — the management account consolidates and pays the bill.
...
- **Sharing Reserved Instances and Savings Plans**: a Reserved Instance or Savings Plan bought in one account but not fully used can be picked up by other accounts in the organization, so no commitment goes to waste. It's **on** by default for every member account, and can be turned off per account from the management account.
...
- When you assume an IAM Role (via STS `AssumeRole`), you **give up** your original permissions and put on the role's permission set for the duration of the session. It's like becoming a completely new person, with no memory of who you used to be.
