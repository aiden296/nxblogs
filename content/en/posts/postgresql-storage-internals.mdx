...
- **FSM fork** (`<relfilenode>_fsm`): the Free Space Map.
- **VM fork** (`<relfilenode>_vm`): the Visibility Map.
...
- **`t_xmax` (4 B)** — the XID that DELETEd or UPDATEd this tuple. `0` means the tuple is live, not yet touched by anyone. When you `DELETE` a row, PostgreSQL only sets `t_xmax = current_xid` and leaves the tuple in place — this is the essence of MVCC.
...
When a tuple exceeds this threshold, TOAST kicks in: PostgreSQL compresses variable-length columns or relocates them to a side table, leaving only an inline pointer behind.
