...
<ExcalidrawDiagram
  ...
  alt="AWS DataSync architecture. On the left, an On-Premises group contains an NFS or SMB server connected to an AWS DataSync agent over NFS or SMB. The agent transfers data over an encrypted TLS connection to the AWS DataSync service inside an AWS Region. From there, DataSync writes to AWS Storage Resources: Amazon S3 across storage classes (S3 Standard, S3 Intelligent-Tiering, S3 Standard-IA, S3 One Zone-IA, S3 Glacier, S3 Glacier Deep Archive), Amazon EFS, and Amazon FSx. Arrows are bidirectional, showing DataSync can move data in either direction."
/>
