...
- `UUID v4` gives every row a value drawn uniformly across the entire key space. Two rows `INSERT`ed back-to-back in time (and therefore physically adjacent on the heap) have UUIDs **wildly far apart** in the index → correlation near 0.
