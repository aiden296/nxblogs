...
```
src: 10.0.0.20   ← IP of the EC2 in the private subnet
dst: 50.60.4.10  ← IP of an external server
```

The NAT Instance has IP `10.0.0.30`. Neither the source nor the destination matches the NAT Instance's IP. With Source/Destination Check enabled, this packet gets dropped — the NAT Instance never even sees it.

Similarly, when the NAT Instance forwards a packet outbound, the original source IP (`10.0.0.20`) doesn't match the NAT Instance's IP → check fails → packet dropped again.
...
| Entity                       | IP                                            |
| ---------------------------- | --------------------------------------------- |
| EC2 (private subnet)         | `10.0.0.20`                                   |
| NAT Instance (public subnet) | Private: `10.0.0.30`, EIP: `12.34.56.78`      |
| External server              | `50.60.4.10`                                  |

**Outbound:**

| #   | Location              | src                          | dst           | Action                                              |
| --- | --------------------- | ---------------------------- | ------------- | --------------------------------------------------- |
| 1   | **EC2**               | `10.0.0.20`                  | `50.60.4.10`  | Create packet                                       |
| 2   | **Route Table** (private) | `10.0.0.20`              | `50.60.4.10`  | Match `0.0.0.0/0 → eni-xxx` → forward to NAT Instance |
| 3   | **NAT Instance**      | `10.0.0.20` → `10.0.0.30`   | `50.60.4.10`  | iptables replaces src IP, saves mapping in conntrack |
| 4   | **Route Table** (public) | `10.0.0.30`               | `50.60.4.10`  | Match `0.0.0.0/0 → igw-xxx` → forward to IGW        |
| 5   | **IGW**               | `10.0.0.30` → `12.34.56.78` | `50.60.4.10`  | 1:1 NAT private → EIP                               |
| 6   | **Internet**          | `12.34.56.78`                | `50.60.4.10`  | Server receives request                              |

**Return:**

| #   | Location              | src           | dst                          | Action                                 |
| --- | --------------------- | ------------- | ---------------------------- | -------------------------------------- |
| 1   | **Server**            | `50.60.4.10`  | `12.34.56.78`                | Send response                          |
| 2   | **IGW**               | `50.60.4.10`  | `12.34.56.78` → `10.0.0.30` | 1:1 NAT EIP → private                 |
| 3   | **Route Table** (public) | `50.60.4.10` | `10.0.0.30`               | Match `10.0.0.0/16 → local`           |
| 4   | **NAT Instance**      | `50.60.4.10`  | `10.0.0.30` → `10.0.0.20`   | Look up conntrack, replace dst IP      |
| 5   | **Route Table** (private) | `50.60.4.10` | `10.0.0.20`              | Match `10.0.0.0/16 → local`           |
| 6   | **EC2**               | `50.60.4.10`  | `10.0.0.20`                  | Receive response                       |
...
| ----- | ---- | ------------------------------------- | ---------------------------------------- |
| HTTP  | 80   | `10.0.0.0/24` (Private Subnet CIDR)  | Allow HTTP traffic from private subnet   |
| HTTPS | 443  | `10.0.0.0/24` (Private Subnet CIDR)  | Allow HTTPS traffic from private subnet  |
...
**Important note about High Availability**: NAT Gateway has automatic HA **within a single AZ**. If that AZ goes down, the NAT Gateway goes down too. For region-wide HA, you need to deploy **one NAT Gateway per AZ** and configure route tables accordingly for each AZ's private subnets.
