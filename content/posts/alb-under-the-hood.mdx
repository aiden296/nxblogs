...
Client request
    ...
    ▼
┌──────────────────────────────────┐
│ Quyết định 1: Chọn ALB Node      │  ← DNS shuffle (client-side)
│ "Node nào handle request?"       │     Không config được
└──────────────────────────────────┘
    │
    ▼ (TCP connection đến node X)
┌──────────────────────────────────┐
│ Quyết định 2: Chọn Target        │  ← Round Robin / LOR / Weighted
│ "Backend nào xử lý?"             │     Config ở target group
└──────────────────────────────────┘
    │
    ▼
Backend
...
```
Trạng thái target group hiện tại:
  Target A: đang xử lý 8 requests
  Target B: đang xử lý 3 requests   ← chọn B
  Target C: đang xử lý 5 requests

Request mới đến → ALB chọn Target B
```
...
Trạng thái bình thường:
  A, B, C cùng weight 1.0 → mỗi target ~33% traffic

Target B đột nhiên có 5xx rate cao bất thường:
  ALB tự giảm weight của B xuống 0.1
  → A: 47%, B: 5%, C: 47%
  
Khi B "phục hồi" (5xx về bình thường):
  ALB tự nâng weight B trở lại 1.0
```
