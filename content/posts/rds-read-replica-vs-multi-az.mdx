...
Client ──► Primary
           1. Ghi vào local
           2. Trả "OK" về Client NGAY
           3. (Sau đó) gửi log sang Replica trong nền
                                            │
                                            ▼
                                    Replica nhận và apply
                                    (sau vài ms → vài giây)
```
...
```
        ...
        │   App / EC2  │
        └──────┬───────┘
               │ (DNS endpoint duy nhất)
               ▼
       ┌──────────────────┐         sync replication        ┌──────────────────┐
       │  Primary (AZ-A)  │ ──────────────────────────────► │  Standby (AZ-B)  │
       │   read + write   │                                 │  KHÔNG phục vụ   │
       └──────────────────┘                                 └──────────────────┘
```
...
```
Không Multi-AZ:  Client ──► Primary ──► OK    (1 hop)
Có Multi-AZ:     Client ──► Primary ──► Standby ──► ack ──► OK    (3 hop)
```
