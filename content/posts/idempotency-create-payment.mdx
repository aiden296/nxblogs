...
## 1. Nguồn gốc: Từ đại số đến hệ phân tán

### 1.1. Năm 1870 — Benjamin Peirce và phép nhân "tự thân"

Thuật ngữ **idempotent** được nhà toán học người Mỹ **Benjamin Peirce** đặt ra trong tác phẩm _"Linear Associative Algebra"_ (1870). Ông gọi một phần tử `x` là idempotent nếu:

```
x · x = x
```

Tức là nhân nó với chính nó vẫn ra chính nó. Từ "idempotent" có gốc Latin: _idem_ (cùng) + _potent_ (lực, khả năng) — nghĩa đen là "vẫn cùng một sức mạnh" dù áp dụng bao nhiêu lần.

Vài ví dụ quen thuộc:

- `1 · 1 = 1`, `0 · 0 = 0`
- `abs(abs(x)) = abs(x)` — lấy giá trị tuyệt đối hai lần vẫn ra cùng kết quả
- `A ∪ A = A` — hợp của một tập với chính nó vẫn là chính nó
- `max(max(x, y), max(x, y)) = max(x, y)`

### 1.2. Computer Science: từ phép toán đến thao tác

Khi khái niệm này du nhập vào CS, nó được tổng quát hóa: một **thao tác** (operation, function, API call) được gọi là idempotent nếu:
...
### 1.3. HTTP — RFC 2616 (1999) → RFC 9110 (2022)
...
### 1.4. Distributed systems era: Stripe và Idempotency-Key
...
Dưới đây là các pitfall điển hình tôi đã thấy trong code review và post-mortem:

| Pitfall                        | Hậu quả                                                                              | Cách tránh                                                    |
| ------------------------------ | ------------------------------------------------------------------------------------ | ------------------------------------------------------------- |
| Không validate request hash    | Client lỡ tay reuse key cho request khác → trả nhầm response của payment cũ          | Hash body, so sánh mỗi lần (như `requestHash` ở trên)         |
| Cache cả `5xx` response        | Lỗi tạm thời (DB blip, network) thành "vĩnh viễn"; mọi retry đều trả lại cùng lỗi đó | Chỉ cache 2xx và 4xx deterministic; với 5xx phải `del` lock   |
| Quên release lock khi fail     | Sau lỗi, client retry sẽ bị `409` cho đến khi TTL hết                                | Luôn `redis.del(redisKey)` trong `catch`                      |
| TTL quá ngắn (vd. 5 phút)      | Mobile offline 2 tiếng rồi retry → key đã expire → charge lần 2                      | Dùng TTL ≥ 24h theo chuẩn Stripe                              |
| Dùng timestamp/counter làm key | Trùng key giữa 2 user khác nhau → cross-user response leak                           | Bắt client sinh **UUID v4**; reject key không hợp format      |
| Không scope key theo endpoint  | Cùng key dùng cho `/payments` và `/refunds` đụng nhau                                | Prefix key trong Redis: `idem:<endpoint>:<key>`               |
| Redis fail-open                | Redis down → bypass check → double-charge khi retry                                  | Treat Redis lỗi = `503 Service Unavailable`; **không** bypass |

Có một edge case sâu hơn đáng lưu ý: **idempotency key vs DB transaction**. Trong code mẫu trên, ta lưu key vào Redis, còn payment thì lưu vào DB (qua `chargeCustomer`). Nếu Redis ghi xong mà DB rollback (hoặc ngược lại), trạng thái sẽ lệch. Production-grade systems thường persist idempotency key **vào cùng DB transaction** với payment record — nhưng đó là một topic riêng, có thể bàn ở post sau.
