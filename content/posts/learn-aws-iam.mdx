...
AWS khuyến nghị dùng **IAM Identity Center**. Thay vì giữ một chiếc chìa khóa vĩnh viễn, bạn đăng nhập qua một Portal, nhận một **Temporary Credential** (chỉ có tác dụng trong 1-12 tiếng).

* **Với Developer:** Thay vì lưu Key cố định, hãy dùng lệnh `aws sso login`. SDK sẽ tự động hiểu và lấy quyền truy cập mà không cần lưu trữ thông tin nhạy cảm trong code.
