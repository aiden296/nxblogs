...
Pool khởi tạo:
  [TCP+TLS+Auth] → C1 ─┐
  [TCP+TLS+Auth] → C2 ─┤
  [TCP+TLS+Auth] → C3 ─┼──► Idle Pool [C1, C2, C3, C4, C5]
  [TCP+TLS+Auth] → C4 ─┤
  [TCP+TLS+Auth] → C5 ─┘


Khi có request:
  Request → checkout C1 → execute query (2ms) → release C1 về pool
  Request → checkout C2 → execute query (3ms) → release C2 về pool

Không có TCP/TLS/Auth overhead. Tổng thời gian ≈ thời gian query.
```
...
```
┌─────────────┐
│ Instance 1  │──── Local Pool (max=20) ────┐
├─────────────┤                             │
│ Instance 2  │──── Local Pool (max=20) ────┤
├─────────────┤                             │
│ Instance 3  │──── Local Pool (max=20) ────┤
├─────────────┤                             ├──► PostgreSQL
│ Instance 4  │──── Local Pool (max=20) ────┤    max_connections = 100
├─────────────┤                             │
│ Instance 5  │──── Local Pool (max=20) ────┤    ❌ 5 × 20 = 100 → FULL
├─────────────┤                             │
│ Instance 6  │──── Local Pool (max=20) ────┘    ❌ Instance 6 → REJECTED
└─────────────┘
```
...
```
┌─────────────┐
│ Instance 1  │──── Local Pool ────┐
├─────────────┤                    │
│ Instance 2  │──── Local Pool ────┤
├─────────────┤                    │
│ Instance 3  │──── Local Pool ────┤
├─────────────┤                    ├──► RDS Proxy ──► PostgreSQL
│ Instance 4  │──── Local Pool ────┤    (giữ ~50       max_connections=100
├─────────────┤                    │     DB connections)
│    ...      │                    │
├─────────────┤                    │
│ Instance 10 │──── Local Pool ────┘
└─────────────┘

App connections: 10 × 20 = 200
DB connections:  ~50 (do RDS Proxy multiplex)
```
